...
impl Epd5in79Impl {
    ...
    pub fn set_binary(&mut self, Pixel(point, color): Pixel<BinaryColor>) {
    ...
    pub fn get_binary(&self, Point { x, y }: Point) -> Option<BinaryColor> {
    ...
    pub fn set_gray(&mut self, Pixel(point, color): Pixel<Gray2>) {
        if !is_point_in_screen(point) {
            return;
        }
    ...
    pub fn get_gray(&self, Point { x, y }: Point) -> Option<Gray2> {
        if !is_point_in_screen(Point::new(x, y)) {
            return None;
        }
